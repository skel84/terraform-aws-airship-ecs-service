...
resource "aws_lb_listener_rule" "host_based_routing" {
  ...
  condition {
    host_header {
      values = [
        local.route53_record_type == "CNAME" ? join("", aws_route53_record.record.*.fqdn) : join("", aws_route53_record.record_alias_a.*.fqdn),
      ]
    }
  }
}
...
resource "aws_lb_listener_rule" "host_based_routing_custom_listen_host" {
  ...
  condition {
    host_header {
      values = [data.template_file.custom_listen_host[count.index].rendered]
    }
  }
}
